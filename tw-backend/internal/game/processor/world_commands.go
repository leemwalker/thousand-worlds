...
func (p *GameProcessor) handleWorldSimulate(ctx context.Context, client websocket.GameClient, argsStr string) error {
	...
	for year < years {
		...
		if year-lastProgress >= progressInterval && progressInterval > 0 {
			...
			// Send map update to client every progress interval for visual feedback
			// Only send if we're past 0% and before 100% (final update happens at end)
	...
	log.Printf("[WorldSimCmd] Satellites count before send: %d", len(satellites))
	if len(satellites) > 0 {
		var rings interface{} = nil
		if geology.Rings != nil {
			rings = geology.Rings.GetVisibleRings()
		}
		log.Printf("[WorldSimCmd] Sending satellites_info with %d satellites", len(satellites))
		...
		log.Printf("[WorldSimCmd] WARNING: No satellites to send!")
	}
